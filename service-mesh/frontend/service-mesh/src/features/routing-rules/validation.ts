import type { Destination, RuleFormValues } from './types.js'
...
export const validateWeights = (destinations: Destination[]): ValidationResult => {
  ...
  if (sum !== 100) {
    return fail([{ field: 'destinations', message: `Сумма весов = ${sum}%, должно быть 100%` }])
  }
  return ok()
}
...
export const validateRule = (values: RuleFormValues): ValidationResult => {
  ...
  if (!weightsResult.ok) {
    errors.push(...weightsResult.errors)
  }
