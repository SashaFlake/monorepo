    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-faint)', marginLeft: 'auto' }}>
          generated {new Date(m.metadata.generatedAt).toLocaleTimeString()}
        </span>
      </div>

      {/* Spec grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 'var(--space-3)' }}>
        <SpecCard title="Exposure">
          <KV k="exposure" v={m.spec.exposure} />
          <KV k="protocol" v={m.spec.protocol} />
        </SpecCard>
        <SpecCard title="Ports">
          {m.spec.ports.map(p => (
            <KV key={p.name} k={p.name} v={`${p.port} → ${p.targetPort} (${p.protocol})`} />
          ))}
        </SpecCard>
        <SpecCard title="Routing">
          <KV k="load balancing" v={m.spec.routing.loadBalancing} />
          <KV k="retries" v={String(m.spec.routing.retries)} />
          <KV k="timeout" v={`${m.spec.routing.timeoutMs}ms`} />
        </SpecCard>
        <SpecCard title="Health">
          <KV k="path" v={m.spec.health.path} />
          <KV k="interval" v={`${m.spec.health.intervalMs / 1000}s`} />
          <KV k="ttl" v={`${m.spec.health.ttlMs / 1000}s`} />
        </SpecCard>
      </div>
    </div>
  )
...
function OpenApiPanel({ serviceId, version }: { serviceId: string; version: string }) {
  ...
  if (isLoading) return (
    <div style={{ color: 'var(--color-text-faint)', fontSize: 'var(--text-sm)', padding: 'var(--space-4)' }}>
      Fetching OpenAPI from instance…
    </div>
  )
  ...
  if (isError) return (
    <div style={{
      ...
      <div style={{ color: 'var(--color-text-muted)', fontFamily: 'monospace', fontSize: 'var(--text-xs)' }}>
        {(error as Error).message}
      </div>
      <div style={{ marginTop: 'var(--space-2)', fontSize: 'var(--text-xs)', color: 'var(--color-text-faint)' }}>
        Make sure the instance exposes <code>/openapi.json</code>
      </div>
    </div>
  )
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {routes.map((r, i) => (
              <tr
                key={`${r.method}-${r.path}`}
                style={{ borderBottom: i < routes.length - 1 ? '1px solid var(--color-divider)' : 'none', opacity: r.deprecated ? 0.5 : 1 }}
              >
                <td style={tdStyle}>
                  <span style={{
                    fontFamily: 'monospace',
                    fontSize: 'var(--text-xs)',
                    fontWeight: 700,
                    color: HTTP_METHOD_COLOR[r.method.toLowerCase()] ?? 'var(--color-text)',
                    background: 'var(--color-surface-offset)',
                    padding: '2px 6px',
                    borderRadius: 'var(--radius-sm)',
                  }}>{r.method}</span>
                ...
                <td style={{ ...tdStyle, fontFamily: 'monospace', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>
                  {r.path}{r.deprecated && <span style={{ color: 'var(--color-warning)', marginLeft: 4 }}>deprecated</span>}
                </td>
...
function InstancesPanel({ version }: { version: ServiceVersion }) {
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
          {['ID', 'Host', 'Port', 'Health', 'Status', 'Last heartbeat'].map(h => (
    ...
    <Card style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <span style={{
          ...
          {version.instanceCount} instance{version.instanceCount !== 1 ? 's' : ''}
        </span>
      </div>
      ...
      <Header
        ...
        subtitle={
          <span style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <Link
              to="/services"
              style={{ color: 'var(--color-text-faint)', fontSize: 'var(--text-sm)', textDecoration: 'none' }}
            >
              Services
            </Link>
      ...
      <main style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        ...
        {!isLoading && !isError && (data?.versions.length ?? 0) === 0 && (
          <Card style={{ textAlign: 'center', padding: 'var(--space-12)', color: 'var(--color-text-faint)', fontSize: 'var(--text-sm)' }}>
            No instances registered — no versions to show.
          </Card>
        )}

        {data?.versions.map(v => (
          <VersionCard key={v.version} version={v} serviceId={serviceId} />
        ))}
      </main>
    </>
  )
