...
**MDM Core** — сердце системы. Знает какие устройства должны быть в системе, принимает новые, отслеживает их состояние и доставляет на них управляющие воздействия. Отвечает внешним системам на вопрос о доверии через `/external/devices/:id/compliance` — эндпоинт read-only, читает из той же базы. Это единый сервис, а не несколько — потому что это один домен: устройство, политика, команда и процесс принятия в систему не существуют друг без друга.
...
Compliance Service — если внешних систем станет много или compliance-запросы начнут влиять на нагрузку Core, выносится в отдельный сервис с собственным стором, который обновляется по событиям из Kafka. До этого момента — эндпоинт в Core.
